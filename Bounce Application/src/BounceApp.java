 ...
 *  =============================================================
 *  Coder: Jacky Shew
 *  BounceApp.java : Extends JFrame and contains a panel where
...
public class BounceApp extends JFrame {
	...
	* @return toolsPanel		the Panel
	...
	public JPanel setUpToolsPanel() {
    	...
    	ImageIcon crescentButtonIcon = createImageIcon("crescent.gif");
		...
		JComboBox<ImageIcon> shapesComboBox = new JComboBox<ImageIcon>(new ImageIcon[] {rectangleButtonIcon,
				squareButtonIcon, crescentButtonIcon, rotatingSquareButtonIcon} );
		...
		JComboBox<ImageIcon> pathComboBox = new JComboBox<ImageIcon>(new ImageIcon[] {fallingButtonIcon, 
				jumpingButtonIcon, bouncingButtonIcon});
		...
		penWidthTxt.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					int newValue = Integer.parseInt(tf.getText());
					if (newValue > 0) // if the value is valid, then change the current pen width
					 	panel.setCurrentPenWidth(newValue);
					else
						tf.setText(panel.getCurrentPenWidth()+"");
				} catch (Exception ex) {
					tf.setText(panel.getCurrentPenWidth()+""); //if the number entered is invalid, reset it
				}
			}
		});
	...
	protected static ImageIcon createImageIcon(String filename) {
		java.net.URL imgURL = BounceApp.class.getResource(filename);
